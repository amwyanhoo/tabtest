...
return _32=="true";
},message:"Please fix this field."},IP : {
	validator : function(value, p) {
		var ipReg = /^(\d+)\.(\d+)\.(\d+)\.(\d+)$/;
		return ipReg.test(value);
},message : "请输入正确IP"},number: {validator: function (value, param) {return /^[0-9]+.?[0-9]*$/.test(value);
},message: '请输入数字'},lengthStipulate:{validator:function(value,param){return value.length == param;
},message:"长度必须是32位"}}};
})(jQuery);
